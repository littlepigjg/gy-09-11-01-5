...
- **异常点检测**：滑动窗口 Z-Score 算法，曲线上红色高亮数据点标注
- **异常事件聚类**：按时间接近度 + 实例关联性将分散异常点自动聚合为故障事件（起止时间/涉及指标/影响范围），聚类粒度随采样密度自适应
...
| GET | `/api/anomaly-events` | 异常事件聚类（`start/end/metrics/instance/gap`） |
| GET | `/api/health` | 健康检查 |
...
**异常事件聚类**：在异常检测输出之上，按"时间接近度 + 实例关联性"做单链聚类，将分散异常点聚合为异常事件（一次完整故障过程：起止时间、涉及指标列表、影响范围实例）。聚类间隔阈值 = 采样间隔中位数 × 系数（限制在 30s~1800s），随数据密度动态调整——稀疏分布的异常各自独立成事件，同一故障引发的连锁异常（同实例阈值宽松、跨实例要求时间更紧密）被正确关联。每次请求在同一事务快照内完成取数与聚类、不做缓存，新数据流入导致边界异常点变化时，下次查询自然得到新分组；范围内无异常点时返回空事件列表。前端以色带标出事件区间并列表展示。
